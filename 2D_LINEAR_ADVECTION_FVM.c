...
double*** InitialConditions(double*** u_array, double*** u_Exact, double** x_centroid_array, double** y_centroid_array, double** dx_array, double** dy_array, int ARRAY_SIZE_X, int ARRAY_SIZE_Y, int rkStep, int INITIAL_CONDITIONS, FILE* IC_fp, FILE* IC2_fp);
...
void updateGhostCells(double*** u_array, int ARRAY_SIZE_X, int ARRAY_SIZE_Y, int NX, int NY, int N_GHOST, int rkStep);
...
void fluxCalculator(double** uEast, double** uWest, double** uNorth, double** uSouth, double** total_flux, double** dx_array, double** dy_array, int NX, int NY, int N_GHOST, double INITIAL_VELOCITY, int ARRAY_SIZE_X, int ARRAY_SIZE_Y);
...
void ErrorCaclulator(double*** u_array, double*** u_Exact, int NX, int NY, int N_GHOST);
...
int main()
{
    ...
    int N_GHOST = 4;
    int N_RUNGE_KUTTA_STEPS = 1;
    int INITIAL_CONDITIONS = 1;
    int NUMERICAL_METHOD = 1;
    int SLOPE_LIMITER = 1;
    double STOPPING_TIME = 6.0;
    ...
    double** uNorth = (double**)calloc(ARRAY_SIZE_X, sizeof(double*));
    double** uSouth = (double**)calloc(ARRAY_SIZE_X, sizeof(double*));

    double** dx_array = (double**)calloc(ARRAY_SIZE_X, sizeof(double*));
    double** dy_array = (double**)calloc(ARRAY_SIZE_X, sizeof(double*));
    double** x_centroid_array = (double**)calloc(ARRAY_SIZE_X, sizeof(double*));
    double** y_centroid_array = (double**)calloc(ARRAY_SIZE_X, sizeof(double*));

    double** total_flux = (double**)calloc(ARRAY_SIZE_X, sizeof(double*));

    for (int i = 0; i < ARRAY_SIZE_X; i++)
    {
        u_array[i] = (double**)calloc(ARRAY_SIZE_Y, sizeof(double*));
        u_Exact[i] = (double**)calloc(ARRAY_SIZE_Y, sizeof(double*));

        dx_array[i] = (double*)calloc(ARRAY_SIZE_Y, sizeof(double));
        dy_array[i] = (double*)calloc(ARRAY_SIZE_Y, sizeof(double));
        x_centroid_array[i] = (double*)calloc(ARRAY_SIZE_Y, sizeof(double));
        y_centroid_array[i] = (double*)calloc(ARRAY_SIZE_Y, sizeof(double));

        uEast[i] = (double*)calloc(ARRAY_SIZE_Y, sizeof(double));
        uWest[i] = (double*)calloc(ARRAY_SIZE_Y, sizeof(double));
        uNorth[i] = (double*)calloc(ARRAY_SIZE_Y, sizeof(double));
        uSouth[i] = (double*)calloc(ARRAY_SIZE_Y, sizeof(double));

        total_flux[i] = (double*)calloc(ARRAY_SIZE_Y, sizeof(double));
    ...
    u_Exact = InitialConditions(u_array, u_Exact, x_centroid_array, y_centroid_array, dx_array, dy_array, ARRAY_SIZE_X, ARRAY_SIZE_Y, rkStep, INITIAL_CONDITIONS, IC_fp, IC2_fp);
    ...
    //     //printf("\n");
         ...
         {
             updateGhostCells(u_array, ARRAY_SIZE_X, ARRAY_SIZE_Y, NX, NY, N_GHOST, rkStep);
             ...
             fluxCalculator(uEast, uWest, uNorth, uSouth, total_flux, dx_array, dy_array, NX, NY, N_GHOST, INITIAL_VELOCITY_X, ARRAY_SIZE_X, ARRAY_SIZE_Y);
    ...
    printf("\n");
    ErrorCaclulator(u_array, u_Exact, NX, NY, N_GHOST);
            ...
            case 1:
                ...
                for (int i = N_GHOST - 1; i < NX + N_GHOST + 1; i++)
                {
                    for (int j = N_GHOST -1; j < NY +N_GHOST + 1; j++)
                    {
                        ...
                        uEast[i][j] = 0.5 * (u_array[i][j][rkStep] * u_array[i][j][rkStep]);
                        uWest[i][j] = 0.5 * (u_array[i][j][rkStep] * u_array[i][j][rkStep]);
                        uNorth[i][j] = 0.5 * (u_array[i][j][rkStep] * u_array[i][j][rkStep]);
                        uSouth[i][j] = 0.5 * (u_array[i][j][rkStep] * u_array[i][j][rkStep]);
                    }
                }
                break;
...
void fluxCalculator(double** uEast, double** uWest, double** uNorth, double** uSouth, double** total_flux, double** dx_array, double** dy_array, int NX, int NY, int N_GHOST, double INITIAL_VELOCITY, int ARRAY_SIZE_X, int ARRAY_SIZE_Y)
{
    ...
    for (int i = N_GHOST; i < NX + N_GHOST; i++)
    {
        ...
        for (int horizontalEdgeIndex = N_GHOST; horizontalEdgeIndex < NX + N_GHOST + 1; horizontalEdgeIndex++ )
        {
            //temporary variables to store the indices for the lax fredrichs flux formula
        ...
        case 1:
            for (int i = N_GHOST; i < NX + N_GHOST; i++)
            {
                for (int j = N_GHOST; j < NY + N_GHOST; j++)
                {
                    ...
                    u_array[i][j][rkStep + 1] = u_array[i][j][rkStep] + dt / dx_array[0][0] * total_flux[i][j];
...
void ErrorCaclulator(double*** u_array, double*** u_Exact, int NX, int NY, int N_GHOST)
{
    ...
    for (int i = N_GHOST; i < NX + N_GHOST; i++)
    {
        for (int j = N_GHOST; j < NY + N_GHOST; j++)
        {
            double error = fabs(u_Exact[i][j][0] - u_array[i][j][0]);
            errorL1 += error;
            errorL2 += error * error;
    ...
    printf("%f  %f  %f\n", errorL1, errorL2, errorLinf);

    double ORDER_0F_ACCURACY = (log(11.92) - log(5.78)) / (log(200) - log(100));
    printf("%f", ORDER_0F_ACCURACY);

   // 5.859537  2.224445  0.959001
    // 12.086057  3.188160  0.958074
}
